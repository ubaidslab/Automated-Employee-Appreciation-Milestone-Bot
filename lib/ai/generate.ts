...
/**
 * Same provider selection convention as the sibling AI Customer Support
 * Agent project (AI_PROVIDER env var, Cloudflare default). Deliberately
 * NOT streaming here, unlike that project's chat route — this runs inside a
 * cron job and a form submit, not a live UI waiting on tokens, so a single
 * awaited response is the right tool, not a copy-pasted streaming pipeline.
 */
